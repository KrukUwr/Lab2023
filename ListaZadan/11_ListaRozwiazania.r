...
Payments <- Events[Month <= 6,.(P6M = sum(ifelse(is.na(PaymentAmount),0,PaymentAmount))), by=.(CaseId)]
setkey(Payments,CaseId)

Cases <- Cases[Payments[,.(CaseId,P6M)],nomatch=0][,target:=ifelse(P6M>300,1,0)]
